  ...
  (testing "Predicates"
    (are [expected got] (= expected got)
         true (every? true? (map graph? [(graph [1 2])
                                         ...
                                         #_{:clj-kondo/ignore [:missing-protocol-method]} (reify Graph)]))
         true (every? true? (map directed? [(digraph [1 2])
                                            (weighted-digraph [1 2])
                                            (fly-graph :predecessors [1 2])
                                            #_{:clj-kondo/ignore [:missing-protocol-method]} (reify Digraph)]))
         true (every? true? (map weighted? [(weighted-graph [1 2])
                                            (weighted-digraph [1 2])
                                            (fly-graph :weight (constantly 1))
                                            #_{:clj-kondo/ignore [:missing-protocol-method]} (reify WeightedGraph)]))))
  (testing "Adders"
